	...
	public static void main(String[] args) {
		...
		try {
			...
			if (imageWidth1 == imageHeight1 && imageWidth2 == imageHeight2) {
				if (!MathematicalOperations.checkLogIsInteger(imageWidth1, 2)
						&& !MathematicalOperations.checkLogIsInteger(imageWidth2, 2)) {
			...
			parentNode1 = quadtree.quadTree(img1, new Coordinate(0, 0),
					new Coordinate(img1.getWidth(), img1.getHeight()), img1.getHeight(), img1.getWidth());

			parentNode2 = quadtree.quadTree(img2, new Coordinate(0, 0),
					new Coordinate(img2.getWidth(), img2.getHeight()), img1.getHeight(), img1.getWidth());
...
class QuadTree {
	...
	public QuadTreeNode<Colour> quadTree(BufferedImage img, Coordinate topLeft, Coordinate bottomRight, int height,
			int width) {
		...
		if (height == 1 && width == 1) {
			...
			QuadTreeNode<Colour> northWestNode = this.quadTree(img, new Coordinate(topLeft.x, topLeft.y),
					new Coordinate(width / 2, height / 2), height / 2, width / 2);

			QuadTreeNode<Colour> southWestNode = this.quadTree(img, new Coordinate(topLeft.x + (width / 2), topLeft.y),
					new Coordinate(width / 2, bottomRight.y), height / 2, width - (width / 2));

			QuadTreeNode<Colour> northEastNode = this.quadTree(img, new Coordinate(topLeft.x, topLeft.y + (height / 2)),
					new Coordinate(bottomRight.x / 2, height / 2), height - (height / 2), width / 2);

			QuadTreeNode<Colour> sountEastNode = this.quadTree(img,
					new Coordinate(topLeft.x + (width / 2), topLeft.y + (height / 2)),
					new Coordinate(bottomRight.x, bottomRight.y), height - (height / 2), width - (width / 2));
